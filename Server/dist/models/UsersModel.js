"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
require('dotenv').config();
const jwt = require('jsonwebtoken');
const bcrypt = require('bcrypt');
const db = require("../db");
const sendVerificationEmail = require("../utils/nodeMailer/sendVerificationEmail");
class UsersModel {
    sendVerificationEmail(req) {
        return __awaiter(this, void 0, void 0, function* () {
            const { user_name, user_email, password } = req.body;
            const saltRounds = 10;
            const salt = yield bcrypt.genSalt(saltRounds);
            const bcryptPassword = yield bcrypt.hash(password, salt);
            const user = {
                user_name: user_name,
                user_email: user_email,
                password: bcryptPassword
            };
            const token = jwt.sign(user, process.env.JWT_SECRET, { expiresIn: "12hr" });
            sendVerificationEmail(user_email, token);
        });
    }
    isUserAuthenticated(req) {
        return __awaiter(this, void 0, void 0, function* () {
            const jwtToken = req.header("token");
            try {
                const user = jwt.verify(jwtToken, process.env.JWT_SECRET, { expiresIn: "2hr" });
                return user;
            }
            catch (error) {
                return false;
            }
        });
    }
    isEmailUnique(req) {
        return __awaiter(this, void 0, void 0, function* () {
            const { email } = req.params;
            try {
                console.log(email);
                const user = yield db.query('SELECT * FROM users WHERE user_email = $1', [email]);
                if (user.rows.length == 0) {
                    return true;
                }
                else {
                    return false;
                }
            }
            catch (error) {
                console.error(error);
            }
        });
    }
    isUsernameUnique(req) {
        return __awaiter(this, void 0, void 0, function* () {
            const { username } = req.params;
            try {
                const user = yield db.query('SELECT * FROM users WHERE user_name = $1', [username]);
                console.log(user.rows);
                if (user.rows.length == 0) {
                    return true;
                }
                else {
                    return false;
                }
            }
            catch (error) {
                console.error(error);
            }
        });
    }
    login(req) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const { user_name, user_email, password } = req.body;
                const user = yield db.query("SELECT * FROM users WHERE user_name = $1 OR user_email = $2;", [
                    user_name, user_email
                ]);
                const userData = { user_id: user.rows[0].user_id, user_name: user.rows[0].user_name };
                const validPassword = yield bcrypt.compare(password, user.rows[0].password);
                if (validPassword) {
                    const token = jwt.sign(userData, process.env.JWT_SECRET, { expiresIn: "2hr" });
                    return token;
                }
            }
            catch (error) {
            }
        });
    }
    createUser(req) {
        return __awaiter(this, void 0, void 0, function* () {
            const user = req.body;
            const saltRounds = 10;
            const salt = yield bcrypt.genSalt(saltRounds);
            const bcryptPassword = yield bcrypt.hash(user.password, salt);
            const duplicateRows = yield db.query("SELECT * FROM users WHERE user_name = $1 OR user_email = $2", [
                user.user_name, user.user_email
            ]);
            if (duplicateRows.rows.length < 1) {
                yield db.query("INSERT INTO users (user_name, user_email, password) VALUES ($1, $2, $3)", [
                    user.user_name, user.user_email, bcryptPassword
                ]);
            }
        });
    }
    createVerifiedUser(user_name, user_email, password) {
        return __awaiter(this, void 0, void 0, function* () {
            const duplicateRows = yield db.query("SELECT * FROM users WHERE user_name = $1 OR user_email = $2", [
                user_name, user_email
            ]);
            if (duplicateRows.rows.length < 1) {
                yield db.query("INSERT INTO users (user_name, user_email, password) VALUES ($1, $2, $3)", [
                    user_name, user_email, password
                ]);
            }
        });
    }
}
module.exports = new UsersModel();
